...
function setup() {
	...
	vCheck.visible = false

	leader = createSprite(200,thor1.y,20,20);
	leader.addImage(leaderImg);
	leader.scale = 0.3
	leader.visible = false
    leaderCheck = createSprite(200,thor1.y,10,100)
    leaderCheck.shapeColor = "blue"
	leaderCheck.visible = false
...
function draw() {
	...
	count =1
	  ...
	  if(p >= 22000 && thor1.x === p){
		  leader.x = thor1.x +450
		  leaderCheck.x = leader.x
        leader.visible = true
		sword.position.x = leader.x-100
	//	swordCheck.x = sword.position.x+100
		Visiblity = 255
	  }
	 // swordCheck.x = sword.position.x
	  ...
	  drawSprites();
				...
				pop();
				swordCheck.y = sword.position.y-40
				swordCheck.x = sword.position.x-150
  ...
  function leaderfight(){
   ...
   //destroying villain
   if(hammerCheck.isTouching(leaderCheck) && leader.visible === true){
   //console.log(hammer.position.x);
   //console.log(v.x)
 // v.visible = false
